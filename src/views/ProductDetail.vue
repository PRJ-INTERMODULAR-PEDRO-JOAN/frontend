  ...
  <MainLayout>
    <main class="product-detail-container" style="padding-top: 50px; min-height: 100vh;">
      <div class="container py-5">
        <button @click="$router.back()" class="btn btn-outline-secondary fw-bold mb-4">&larr; Volver al Catálogo</button>
        ...
        <div v-else-if="product" class="card shadow-lg border-0 mb-5 overflow-hidden product-main-card">
            <div class="row g-0">
                <div class="col-md-6 bg-white d-flex align-items-center justify-content-center p-4 position-relative product-image-wrapper">
                    <div v-if="product.stock <= 0" class="overlay-agotado-detalle"><span class="badge-agotado-detalle">AGOTADO</span></div>
                    <img :src="getImagePath(product.image)" class="img-fluid rounded" :alt="product.name" @error="handleImageError" style="max-height: 400px; object-fit: contain;" :style="product.stock <= 0 ? 'filter: grayscale(100%); opacity: 0.5;' : ''">
                </div>
                ...
                <div class="col-md-6 bg-light product-info-card">
                    <div class="card-body p-5 h-100 d-flex flex-column justify-content-center">
                        <div class="d-flex justify-content-between align-items-start">
                            ...
                            <div class="text-end"><div class="fs-4 text-warning fw-bold">{{ averageRating }} ⭐</div><small class="text-muted">{{ comments.length }} opiniones</small></div>
                        </div>
                        ...
                        <div class="d-flex align-items-center gap-3 mb-4">
                            ...
                            <span v-else-if="product.stock > 0" class="badge bg-warning text-dark fs-6">¡Últimas unidades!</span>
                        ...
                        <div class="d-flex flex-wrap align-items-center gap-3 mb-4 position-relative">
                            <button v-if="auth.user" @click="toggleLike" class="btn btn-lg rounded-pill px-4 transition-btn fw-bold" :class="isLiked ? 'btn-danger' : 'btn-outline-danger'">
                            ...
                            <button v-if="canAddToCart" @click="handleAddToCart" class="btn btn-primary btn-lg px-5 rounded-pill shadow transition-btn fw-bold">Añadir al Carrito 🛒</button>
                            <button v-else-if="product.stock > 0 && !canAddToCart" class="btn btn-warning btn-lg px-5 rounded-pill shadow text-dark fw-bold" disabled>Máx. Stock Alcanzado ⚠️</button>
    ...
    <transition name="slide-up">
      <div v-if="showStickyCart && product" class="sticky-cart-bar fixed-bottom shadow-lg border-top p-3 z-3">
        <div class="container d-flex justify-content-between align-items-center">
          <div class="d-flex align-items-center gap-3">
            <img :src="getImagePath(product.image)" class="rounded shadow-sm d-none d-sm-block" style="width: 50px; height: 50px; object-fit: cover;">
            <div>
              <h6 class="mb-0 fw-bold text-truncate" style="max-width: 200px;">{{ product.name }}</h6>
              <span class="text-success fw-bold fs-5">{{ formatPrice(product.price) }}</span>
            </div>
          </div>
          <div>
            <button v-if="canAddToCart" @click="handleAddToCart" class="btn btn-primary px-4 py-2 rounded-pill fw-bold shadow">Añadir 🛒</button>
          </div>
        </div>
      </div>
    </transition>
...
const averageRating = computed(() => { if (!comments.value.length) return 0; return (comments.value.reduce((acc, c) => acc + parseInt(c.rating), 0) / comments.value.length).toFixed(1); });
...
const handleScroll = () => { showStickyCart.value = window.scrollY > 450; };
const checkLikeStatus = async (id) => { try { const res = await axios.get(`/api/products/${id}/like`); isLiked.value = res.data.is_liked; if (res.data.likes_count !== undefined) likesCount.value = res.data.likes_count; } catch (e) {} };
...
const loadProductData = async (id) => {
    ...
    try {
        ...
        if (product.value) {
          ...
          if (viewed.length > 4) viewed.pop(); localStorage.setItem('recently_viewed', JSON.stringify(viewed));
        }
        if (auth.user) await checkLikeStatus(id);
    } catch (e) {} finally { loading.value = false; }
};

onMounted(() => { loadProductData(route.params.id); window.addEventListener('scroll', handleScroll); });
...
const toggleLike = async () => { if (!auth.user) return alert("Inicia sesión."); try { const res = await axios.post(`/api/products/${product.value.id}/like`); isLiked.value = res.data.status === 'liked'; likesCount.value = res.data.likes_count; } catch (e) {} };
const submitComment = async () => { try { await axios.post(`/api/products/${product.value.id}/comments`, newComment.value); const res = await axios.get(`/api/products/${product.value.id}/comments`); comments.value = res.data; newComment.value.text = ''; newComment.value.rating = 5; } catch (e) { alert("Error al publicar."); } };
const deleteComment = async (id) => { if(!confirm("¿Borrar?")) return; try { await axios.delete(`/api/comments/${id}`); const res = await axios.get(`/api/products/${product.value.id}/comments`); comments.value = res.data; } catch (e) {} };
...
.sticky-cart-bar { background-color: #ffffff; }
