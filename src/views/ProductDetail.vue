      ...
      <div class="container py-5">
        ...
        <div v-if="loading" class="row g-5 mb-5">
            ...
            <div class="col-md-6 d-flex flex-column justify-content-center">
                ...
                <div class="skeleton-box mb-4" style="height: 40px; width: 40%;"></div>
                <div class="skeleton-box" style="height: 50px; width: 80%; border-radius: 50px;"></div>
            </div>
        </div>
                    ...
                    <div class="card-body p-5 h-100 d-flex flex-column justify-content-center">
                        ...
                        <div class="d-flex flex-wrap align-items-center gap-3 mb-4 position-relative">
                            ...
                            <button 
                                ...
                                class="btn btn-primary btn-lg px-5 rounded-pill shadow transition-btn fw-bold add-to-cart-btn"
                            >
                                Añadir al Carrito 🛒
                            </button>
                            <button 
    ...
    <transition name="slide-up">
      <div v-if="showStickyCart && product" class="sticky-cart-bar fixed-bottom shadow-lg border-top p-3 z-3">
        <div class="container d-flex justify-content-between align-items-center">
          ...
          <div>
            <button v-if="canAddToCart" @click="handleAddToCart" class="btn btn-primary px-4 py-2 rounded-pill fw-bold shadow">
              Añadir 🛒
            </button>
            <button v-else class="btn btn-secondary px-4 py-2 rounded-pill fw-bold" disabled>Agotado</button>
            <transition name="fade">
                <span v-if="showAddedMessage" class="added-notification text-success fw-bold ms-2">✅ ¡Añadido!</span>
            </transition>
          </div>
...
const showStickyCart = ref(false); // Estado de la barra flotante
...
const handleScroll = () => {
  // Muestra la barra si el usuario baja más de 450px
...
onMounted(() => {
    ...
    window.addEventListener('scroll', handleScroll); // Activar escuchador de scroll
});

onUnmounted(() => {
    window.removeEventListener('scroll', handleScroll); // Limpiar al salir
});
...
/* SOPORTE MODO OSCURO PARA SKELETON Y STICKY BAR */
